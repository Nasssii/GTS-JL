...
short MFormWorkStep::Load_Work_ChannelPars(QString channel)
{
    ...
    QSettings *INI_File = new QSettings(QApplication::applicationDirPath() + "/setting/channel.ini", QSettings::IniFormat);
...
void MFormWorkStep::save_File_INI(const QString channel)
{
    ...
    INI_File->destroyed();
    if( ui->Channel->currentText() == m_Form_PrintSetting->ui->CBT_Channel->currentText() ){
        m_Form_PrintSetting->ui->BTN_Save->click();
    }else{
